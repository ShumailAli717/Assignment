...
#define N 1024  // Updated matrix size
...
void gaussianElimination(double** A, double* B, double* X) {
    ...
    // Forward Elimination
    for (k = 0; k < N; k++) {
        #pragma omp parallel for private(i, j) shared(A, B, k) schedule(static,200) num_threads(8)
        for (i = k + 1; i < N; i++) {
    ...
    for (i = N - 1; i >= 0; i--) {
        ...
        #pragma omp parallel for reduction(-:sum) schedule(static,200) num_threads(8)
        for (j = i + 1; j < N; j++) {
            sum -= A[i][j] * X[j];
        }
